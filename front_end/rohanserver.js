...
app.post('/rating', function(req, res) {
  ...
  if (change == 'Update'){
    ...
    if (type = 'Movie'){
      ...
      var sql2 = `UPDATE MovieRating SET value = -1 WHERE name = '${media}'`;
  
  
    }
    else {
      var sql2 = `UPDATE ShowRating SET value = -1 WHERE name = '${media}'`;
  ...
  app.get('/rating-output', function(req, res) {
    res.render('rating-output', { title: 'Ratings' });
  });

  app.get('/rating-success', function(req, res) {
    res.render('rating-success', { title: 'Ratings' });
  });
